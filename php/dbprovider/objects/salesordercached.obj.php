...
class SalesOrder {
    ...
    protected function getCustomerName($custid) {
    	if (!isset($this->custCache[$custid])) {
			return "NOT FOUND";
		}
		return $this->custCache[$custid];
    }
    ...
    protected function extractHeaderRecord($row) {
        ...
        $retObj->custname = $this->getCustomerName($row->custid_h);
        return $retObj;
    }
    ...
    public function findWhere($criteria = "") {
        ...
        $stmt = $this->salesorder->multiJoin(
        	...
        	array(
				new JoinLink("OJ", "h.ordnbr", "d.ordnbr"),
        		new JoinLink("OJ", "h.ordtype", "d.ordtype"),
        		new JoinLink("OJ", "h.bocntr", "d.bocntr"),
...
//         );
        ...
        $this->custCache = array();
        $custDS = $this->db->findWhere("customer", array("custid"=>"%"));
        foreach ($custDS->data as $custRow) {
        	$this->custCache[$custRow->custid] = $custRow->lastname;
        }
        // Build Objects
        $hdrArray = array();
        foreach($stmt as $row) {
