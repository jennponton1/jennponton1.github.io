        ...
        try {
            ...
            if ($configs != '' && is_array($configs)) {
                if (isset($configs['search'])) {
                    $this->searchStmt = $this->db->prepare($configs['search']);
                } // check for search sql;
                if (isset($configs['all'])) {
                    $this->allStmt = $this->db->prepare($configs['all']);
                }
                if (isset($configs['updateItem'])) {
                    $this->updStmt = $this->db->prepare($configs['updateItem']);
                }
                if (isset($configs['insertItem'])) {
                    $this->insStmt = $this->db->prepare($configs['insertItem']);
                }
            }
        } catch (Exception $e) {
            echo "<br>\n";
...
abstract class PDOBaseODBCModelClass extends PDOBaseModelClass {
    ...
    public function __construct($configs = '') {
        try {
            $this->db = new PDO("odbc:" . SOLDSN); //dbname=" . MYSQL_DB . ";host=" . MYSQL_HOST, "root");
...
//        echo "<hr>$sql</hr>";
//        die($sql);
        $this->currentStmt = $this->db->query($sql);
        $this->currentPosition = 0;
        $this->lastFetchedRow = null;
        $this->currentStmt->setFetchMode(PDO::FETCH_ASSOC);
        return $this->currentStmt;
